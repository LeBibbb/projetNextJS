...
export default function Home() {
  ...
  useEffect(() => {
    fetch("https://raw.githubusercontent.com/LeBibbb/monAPI/main/games.json")
      ...
      .then((data) => {
        ...
        setCheapGames(
          cheapGamesData.sort(() => 0.5 - Math.random()).slice(0, 3)
        );
  ...
  return (
    ...
    >
      <header
        className="text-center mb-8 mt-15"
        style={{ backgroundImage: "url('./public/jeux1.webp')" }}
      >
        <h1
          ...
          {games.length > 0 && games[currentIndex] && (
            <div className="relative w-full mx-auto max-w-screen-lg">
              <div className="relative transition-all duration-500 ease-in-out transform">
                <Link
                  href={`/games/${games[currentIndex].id}`}
                ...
                >
                  ...
                  <div
                    className={`absolute bottom-0 left-0 right-0 ${
                      darkMode
                        ? "bg-zinc-800 bg-opacity-50 text-white"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {bestSellers.map((game) => (
                <Link
                  key={game.id}
                  href={`/games/${game.id}`}
                  className={`p-4 rounded-lg shadow-lg text-white transition-transform transform hover:scale-105 ${
